...
var dd=today.getDay()*10000+today.getHours()*100+today.getMinutes();

var diff=0;
var currDayOfWeek = today.getDay();
var nextDayOfWeek = currDayOfWeek +1;
if(nextDayOfWeek == 7)
	nextDayOfWeek = 0;
...
if(today.getMonth()>=9)
aa = today.getDate() +'.' + (today.getMonth()+1) + '.' + today.getFullYear();
var sDayOfweek;
let dayMatrix = ['Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота'];						
sDayOfweek = dayMatrix[today.getDay()];

document.getElementById('before-grid').innerHTML = sDayOfweek + ' ' + aa; // + ' ' + today.getHours() +':' + today.getMinutes();

for(let i=0; i<matrix.length; i++)
{
diff = Math.floor(matrix[i][0]/10000) - currDayOfWeek;
if(diff<0)
	diff = (diff+7);
newToday.setDate(currDate + diff);

if(newToday.getMonth()<9)
{
matrix[i][4] = newToday.getDate() +'.0' + (newToday.getMonth()+1) + '.' + newToday.getFullYear();
}
if(newToday.getMonth()>=9)
{
matrix[i][4] = newToday.getDate() +'.' + (newToday.getMonth()+1) + '.' + newToday.getFullYear();
}
//console.log(Math.floor(matrix[i][0]/10000) + ' ' +  today.getDay());

if(Math.floor(matrix[i][0]/10000) == currDayOfWeek)
matrix[i][4] ='Сегодня';
if(Math.floor(matrix[i][0]/10000) == nextDayOfWeek)
matrix[i][4] ='Завтра';
}

//console.log(matrix);

var dayOfWeek = Math.floor(dd/10000); //day  of weeki 0..6;
var dayOfWeek1 = dayOfWeek+1;
if(dayOfWeek1==7)
dayOfWeek1=0;

//console.log('Sichas = ' + dd + ' DayOfWeek = ' + dayOfWeek);

var ii=0;
var stime=0;
var dayForCol1 = 'Какой то день';
while(true)
{
	if(matrix[ii][0] >= dd)
	{
			dayForCol1 = matrix[ii][4];
			document.getElementById('item00').innerHTML = dayForCol1;

			stime = (matrix[ii][0]%10000).toString();
			stime =   stime.substr(0, 2)+':'+stime.substr(2, 2);
			document.getElementById('item01').innerHTML = stime;
			document.getElementById('item02').innerHTML =matrix[ii][2];
			document.getElementById('item03').innerHTML =matrix[ii][3];

			ii++
			if(ii>lastIndex) 
				ii=0;
			dayForCol1 = matrix[ii][4];
			document.getElementById('item10').innerHTML = dayForCol1;
				
			stime = (matrix[ii][0]%10000).toString();
			stime =   stime.substr(0, 2)+':'+stime.substr(2, 2);
			document.getElementById('item11').innerHTML = stime;
			document.getElementById('item12').innerHTML =matrix[ii][2];
			document.getElementById('item13').innerHTML =matrix[ii][3];

			ii++;
			if(ii>lastIndex) 
				ii=0;
			dayForCol1 = matrix[ii][4]; //'Скоро';
			document.getElementById('item20').innerHTML =dayForCol1;
				
			stime = (matrix[ii][0]%10000).toString();
			stime =   stime.substr(0, 2)+':'+stime.substr(2, 2);
			document.getElementById('item21').innerHTML = stime;	
			document.getElementById('item22').innerHTML =matrix[ii][2];
			document.getElementById('item23').innerHTML =matrix[ii][3];
